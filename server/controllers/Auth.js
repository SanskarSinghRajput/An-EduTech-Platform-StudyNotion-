...
exports.signup = async (req, res) => {
	try {
		...
		// validate krlo ( Check if All Details are there or not ) 
		if (
			!firstName ||
			!lastName ||
			!email ||
		...
		// Find the most recent OTP stored for the user(email)
...
exports.login = async (req, res) => {
	try {
		...
		if (await bcrypt.compare(password, user.password)) {
			...
			// Set (Create) cookie for token and return success response
			const options = {
...
exports.sendotp = async (req, res) => {
	try {
		...
		// If user found with provided email, then return a response
		if (checkUserPresent) {
			// Return 401 Unauthorized status code with error message
			return res.status(401).json({
				success: false,
